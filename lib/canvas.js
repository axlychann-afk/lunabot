...
async function textStrip(text, width = 512) {
    ...
    const fb = await font('open-sans-32-black');
    const lines = wrap(f, String(text || ' ').toUpperCase(), width - 32);
    const lh = measureTextHeight(f, 'Ag', width - 32);
    const h = lines.length * lh + 16;
    const strip = new Jimp({ width, height: h, color: 0x00000000 });
    let y = 8;
    for (const ln of lines) {
        const w = measureText(f, ln);
        const x = Math.round((width - w) / 2);
        strip.print({ font: fb, x: x + 2, y: Math.round(y) + 2, text: ln });
        strip.print({ font: f, x, y: Math.round(y), text: ln });
        y += lh;
    }
...
export async function smemeMp4(inputPath, atas, bawah) {
    ...
    try {
        await new Promise((res, rej) => {
            const p = spawn(ffmpegPath, [
                '-y', '-i', inputPath,
                '-loop', '1', '-framerate', '20', '-i', topP,
                '-loop', '1', '-framerate', '20', '-i', botP,
                '-filter_complex',
                `[0:v]scale=512:512:force_original_aspect_ratio=increase,crop=512:512,setsar=1,fps=20[v];[v][1:v]overlay=0:0:eof_action=pass[v1];[v1][2:v]overlay=0:H-h:eof_action=pass`,
...
export async function smemeWebp(inputBuffer, atas, bawah) {
    ...
    const block = (txt, y) => {
        ...
        let svg = ``;
        lines.forEach((ln, i) => {
            svg += `<text x="256" y="${y + 38 + i * lh}" font-family="Arial, 'DejaVu Sans', sans-serif" font-weight="bold" font-size="${fs_}" fill="white" stroke="black" stroke-width="7" paint-order="stroke" text-anchor="middle">${esc(ln)}</text>`;
        });
        return { svg, h };
    };
