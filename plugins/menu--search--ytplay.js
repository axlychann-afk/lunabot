    ...
    code: async (m, {
        ...
        if (!text) {
            return reply(
                ...
                `*${prefix + command} midnight drive --auto-play*\n` +
                `*${prefix + command} midnight drive --audio-send*`
            );
        }

        const audioSend =
            /(?:^|\s)--audio-send(?:\s|$)/i.test(text);
        ...
        if (!query) {
            return reply("⚠️ Query pencarian tidak boleh kosong.");
        }

        if (
            m.quoted &&
            (
                m.quoted.mtype === 'interactiveMessage' ||
        ...
        try {
            ...
            const searchUrl =
                `https://api.ryuu-dev.my.id/discovery/search/youtube?q=${encodeURIComponent(query)}`;
            ...
            if (!searchRes.ok) {
                throw new Error(
                    `Search API HTTP ${searchRes.status}`
                );
            }
            ...
            if (
                !searchData.success ||
                !Array.isArray(searchData.result?.result) ||
                searchData.result.result.length === 0
            ) {
                await RyuuBotz.sendMessage(m.chat, {
                    react: {
                        text: '❌',
                ...
                return reply(
                    "❌ Video tidak ditemukan atau API error."
                );
            }

            const firstVideo =
                searchData.result.result[0];
            ...
            if (!link) {
                throw new Error(
                    "Link YouTube tidak ditemukan dari hasil pencarian."
                );
            }

            // MENGGUNAKAN SWITCH CASE BREAK DEFAULT UNTUK PENANGANAN MODE
            switch (true) {

                /*
                 * MODE DEFAULT
                 * Menampilkan pilihan button format (Audio/Video).
                 * Aktif juga saat flag lama --select-type dipakai.
                 */
                default: {
                    const ui = new A2UI();
                    const img = ui.image(
                    ...
                    const description = ui.text(title);
                    const cap =
                        `Channel: ${channel}
Durasi: ${duration}
Link: ${link}`;
                    ...
                    await sendA2UIWidget(RyuuBotz, m.chat, {
                        ...
                        buttons: [{
                                name: "quick_reply",
                                params: {
                                    display_text: "💽Audio play",
                                    id: `${prefix}ytmp3 ${link} --auto-play`
                                }
                            },
                            {
                                name: "quick_reply",
                                params: {
                                    ...
                                    id: `${prefix}ytmp3 ${link}`
                                }
                            },
                            {
                                name: "quick_reply",
                                params: {
                                    display_text: "📹Video play",
                                    id: `${prefix}ytmp4 ${link} --auto-play`
                                }
                            },
                            {
                                name: "quick_reply",
                                params: {
                                    display_text: "📹Video download",
                                    id: `${prefix}ytmp4 ${link}`
                                }
                            },
                            {
                                name: "cta_copy",
                ...
                /*
                 * MODE --audio-send
                 * Kirim preview dan audio mpeg langsung.
                 */
                case audioSend: {
                    const finalTitle =
                        title ||
                ...
                /*
                 * MODE --auto-play
                 * Ambil metadata + URL audio, kirim HTML Player
                 * (dahulu mode default).
                 * Audio dikirim via WebSocket binary
                 * (wss://audio.ryuu-dev.my.id/wss/audio?url=...),
                 * bukan lagi Base64.
                 */
                case autoPlay: {
                    const downloadUrl =
